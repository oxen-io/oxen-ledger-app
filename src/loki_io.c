...
void monero_io_set_offset(unsigned int offset) {
    if (offset == IO_OFFSET_END) {
        G_loki_state.io_offset = G_loki_state.io_length;
    } else if (offset < G_loki_state.io_length) {
        G_loki_state.io_offset = offset;
    } else {
...
void monero_io_inserted(unsigned int len) {
    ...
    G_loki_state.io_length += len;
}

void monero_io_discard(int clear) {
    G_loki_state.io_length = 0;
    G_loki_state.io_offset = 0;
    if (clear) {
        monero_io_clear();
    }
}

void monero_io_clear(void) { os_memset(G_loki_state.io_buffer, 0, MONERO_IO_BUFFER_LENGTH); }
...
void monero_io_hole(unsigned int sz) {
    if ((G_loki_state.io_length + sz) > MONERO_IO_BUFFER_LENGTH) {
        THROW(ERROR_IO_FULL);
    }
    os_memmove(G_loki_state.io_buffer + G_loki_state.io_offset + sz,
               G_loki_state.io_buffer + G_loki_state.io_offset,
               G_loki_state.io_length - G_loki_state.io_offset);
    G_loki_state.io_length += sz;
}

void monero_io_insert(unsigned char const* buff, unsigned int len) {
    monero_io_hole(len);
    os_memmove(G_loki_state.io_buffer + G_loki_state.io_offset, buff, len);
    G_loki_state.io_offset += len;
}
...
void monero_io_insert_hmac_for(unsigned char* buffer, int len, int type) {
    ...
    hmac[32] = type;
    if (type == TYPE_ALPHA) {
        hmac[33] = (G_loki_state.tx_sign_cnt >> 0) & 0xFF;
        hmac[34] = (G_loki_state.tx_sign_cnt >> 8) & 0xFF;
        hmac[35] = (G_loki_state.tx_sign_cnt >> 16) & 0xFF;
        hmac[36] = (G_loki_state.tx_sign_cnt >> 24) & 0xFF;
    } else {
        hmac[33] = 0;
        hmac[34] = 0;
        hmac[35] = 0;
    ...
    cx_hmac_sha256(G_loki_state.hmac_key, 32, hmac, 37, hmac, 32);
...
#if defined(IODUMMYCRYPT)
    for (int i = 0; i < len; i++) {
        G_loki_state.io_buffer[G_loki_state.io_offset + i] = buffer[i] ^ 0x55;
    }
#elif defined(IONOCRYPT)
    os_memmove(G_loki_state.io_buffer + G_loki_state.io_offset, buffer, len);
#else
    cx_aes(&G_loki_state.spk, CX_ENCRYPT | CX_CHAIN_CBC | CX_LAST | CX_PAD_NONE, buffer, len,
           G_loki_state.io_buffer + G_loki_state.io_offset, len);
#endif
    G_loki_state.io_offset += len;
    if (G_loki_state.tx_in_progress) {
        monero_io_insert_hmac_for(G_loki_state.io_buffer + G_loki_state.io_offset - len, len,
                                  type);
    }
}
...
void monero_io_insert_u32(unsigned int v32) {
    monero_io_hole(4);
    G_loki_state.io_buffer[G_loki_state.io_offset + 0] = v32 >> 24;
    G_loki_state.io_buffer[G_loki_state.io_offset + 1] = v32 >> 16;
    G_loki_state.io_buffer[G_loki_state.io_offset + 2] = v32 >> 8;
    G_loki_state.io_buffer[G_loki_state.io_offset + 3] = v32 >> 0;
    G_loki_state.io_offset += 4;
}

void monero_io_insert_u24(unsigned int v24) {
    monero_io_hole(3);
    G_loki_state.io_buffer[G_loki_state.io_offset + 0] = v24 >> 16;
    G_loki_state.io_buffer[G_loki_state.io_offset + 1] = v24 >> 8;
    G_loki_state.io_buffer[G_loki_state.io_offset + 2] = v24 >> 0;
    G_loki_state.io_offset += 3;
}

void monero_io_insert_u16(unsigned int v16) {
    monero_io_hole(2);
    G_loki_state.io_buffer[G_loki_state.io_offset + 0] = v16 >> 8;
    G_loki_state.io_buffer[G_loki_state.io_offset + 1] = v16 >> 0;
    G_loki_state.io_offset += 2;
}

void monero_io_insert_u8(unsigned int v8) {
    monero_io_hole(1);
    G_loki_state.io_buffer[G_loki_state.io_offset + 0] = v8;
...
/* ----------------------------------------------------------------------- */
int monero_io_fetch_available(void) { return G_loki_state.io_length - G_loki_state.io_offset; }
void monero_io_assert_available(int sz) {
    if ((G_loki_state.io_length - G_loki_state.io_offset) < sz) {
...
int monero_io_fetch(unsigned char* buffer, int len) {
    ...
    if (buffer) {
        os_memmove(buffer, G_loki_state.io_buffer + G_loki_state.io_offset, len);
    }
    G_loki_state.io_offset += len;
    return len;
}
...
int monero_io_fetch_decrypt(unsigned char* buffer, int len, int type) {
    ...
    if (G_loki_state.tx_in_progress) {
        ...
        monero_io_verify_hmac_for(G_loki_state.io_buffer + G_loki_state.io_offset, len,
                                  G_loki_state.io_buffer + G_loki_state.io_offset + len,
                                  type);
    } else {
        monero_io_assert_available(len);
...
#if defined(IODUMMYCRYPT)
        for (int i = 0; i < len; i++) {
            buffer[i] = G_loki_state.io_buffer[G_loki_state.io_offset + i] ^ 0x55;
        }
#elif defined(IONOCRYPT)
        os_memmove(buffer, G_loki_state.io_buffer + G_loki_state.io_offset, len);
#else  // IOCRYPT
        cx_aes(&G_loki_state.spk, CX_DECRYPT | CX_CHAIN_CBC | CX_LAST | CX_PAD_NONE,
               G_loki_state.io_buffer + G_loki_state.io_offset, len, buffer, len);
#endif
    }
    G_loki_state.io_offset += len;
    if (G_loki_state.tx_in_progress) {
        G_loki_state.io_offset += 32;
    }
    if (buffer) {
        switch (type) {
            case TYPE_SCALAR:
...
int monero_io_fetch_decrypt_key(unsigned char* buffer) {
    ...
    k = G_loki_state.io_buffer + G_loki_state.io_offset;
    // view?
    if (os_memcmp(k, C_FAKE_SEC_VIEW_KEY, 32) == 0) {
        ...
        os_memmove(buffer, G_loki_state.view_priv, 32);
        return 32;
    }
    // spend?
        ...
        os_memmove(buffer, G_loki_state.spend_priv, 32);
        return 32;
    }
    // else
    else {
...
uint64_t monero_io_fetch_varint(void) {
    uint64_t v64;
    G_loki_state.io_offset +=
        monero_decode_varint(G_loki_state.io_buffer + G_loki_state.io_offset,
                             MIN(10, G_loki_state.io_length - G_loki_state.io_offset), &v64);
    return v64;
}
...
unsigned int monero_io_fetch_u32(void) {
    ...
    v32 = ((G_loki_state.io_buffer[G_loki_state.io_offset + 0] << 24) |
           (G_loki_state.io_buffer[G_loki_state.io_offset + 1] << 16) |
           (G_loki_state.io_buffer[G_loki_state.io_offset + 2] << 8) |
           (G_loki_state.io_buffer[G_loki_state.io_offset + 3] << 0));
    G_loki_state.io_offset += 4;
    return v32;
}
...
unsigned int monero_io_fetch_u24(void) {
    ...
    v24 = ((G_loki_state.io_buffer[G_loki_state.io_offset + 0] << 16) |
           (G_loki_state.io_buffer[G_loki_state.io_offset + 1] << 8) |
           (G_loki_state.io_buffer[G_loki_state.io_offset + 2] << 0));
    G_loki_state.io_offset += 3;
    return v24;
}
...
unsigned int monero_io_fetch_u16(void) {
    ...
    v16 = ((G_loki_state.io_buffer[G_loki_state.io_offset + 0] << 8) |
           (G_loki_state.io_buffer[G_loki_state.io_offset + 1] << 0));
    G_loki_state.io_offset += 2;
    return v16;
}
...
int monero_io_do(unsigned int io_flags) {
    ...
    // else send data now
        ...
        G_loki_state.io_offset = 0;
        if (G_loki_state.io_length > MONERO_APDU_LENGTH) {
            THROW(SW_IO_FULL);
        }
        os_memmove(G_io_apdu_buffer, G_loki_state.io_buffer + G_loki_state.io_offset,
                   G_loki_state.io_length);

        if (io_flags & IO_RETURN_AFTER_TX) {
            io_exchange(CHANNEL_APDU | IO_RETURN_AFTER_TX, G_loki_state.io_length);
            return 0;
        } else {
            io_exchange(CHANNEL_APDU, G_loki_state.io_length);
        }
    }
    ...
    G_loki_state.io_offset = 0;
    G_loki_state.io_length = 0;
    G_loki_state.io_protocol_version = G_io_apdu_buffer[0];
    G_loki_state.io_ins = G_io_apdu_buffer[1];
    G_loki_state.io_p1 = G_io_apdu_buffer[2];
    G_loki_state.io_p2 = G_io_apdu_buffer[3];
    G_loki_state.io_lc = 0;
    G_loki_state.io_le = 0;
    G_loki_state.io_lc = G_io_apdu_buffer[4];
    os_memmove(G_loki_state.io_buffer, G_io_apdu_buffer + 5, G_loki_state.io_lc);
    G_loki_state.io_length = G_loki_state.io_lc;
